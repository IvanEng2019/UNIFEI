...
pno deleta(pno arvore, pno anterior, int chave){
    ...
    if(arvore->chave > chave){
        if(deleta(arvore->esq,ant,chave)== ant){
            if(fator_balanceamento(arvore) == 2){
    ...
    if(arvore->chave < chave){
        if(deleta(arvore->dir,ant,chave)== ant){
            if(fator_balanceamento(arvore) == 2){
    ...
    if(arvore->chave == chave){
        if((arvore->esq == NULL) || (arvore->dir == NULL)){
            ...
            if(arvore->esq!=NULL){
             ...
             /*   if(anterior->dir==arvore){
                    arvore=arvore->esq;
                    anterior->dir=arvore;
                }*/
             //   if(anterior->esq==arvore){
           ...
           //deleta(arvore->dir,ant,arvore->chave);
           if(deleta(arvore->dir,ant,arvore->chave)==ant){
            //   if(insere(menor->chave, menor->nome, arvore->dir)== menor){
                   if(fator_balanceamento(arvore) == 2){
                        if(altura(arvore->esq->dir)<= altura(arvore->esq->esq))
                          rotacao_esquerda(arvore);
                        else
