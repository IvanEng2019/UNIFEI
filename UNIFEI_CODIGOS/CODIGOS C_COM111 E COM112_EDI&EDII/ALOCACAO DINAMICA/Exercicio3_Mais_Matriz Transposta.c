...
Data: 12/08/2018
======================================================================================================================
Exercício : matriz transposta: s[i][j]= t[j][i]
...
float *transpostaVetorVp(int m,int n,float *vet);
float **transpostaMatizM(int m,int n,float **mat);
/*************************************************************************************************************************
Função principal
/*************************************************************************************************************************/
int main(void){
    ...
    printf("\n\nAgora fazendo as transpostas...\n");
    tVetBp=transpostaVetorVp(m,n,vetBp);
    printf("\nImprimindo a matriz de vetor transposta :");
    imprimirVetorVet(n,m,tVetBp);

    tMatAp=transpostaMatizM(m,n,matAp);
    printf("\n\nImprimindo a matriz de ponteiro transposta:");
    imprimirMatrizMat(n,m,tMatAp);

    liberaMemoriaVmP(m,matAp,vetBp);
    liberaMemoriaVmP(n,tMatAp,tVetBp);
 return 0;
}
/*************************************************************************************************************************
...
float *transpostaVetorVp(int m,int n,float *vet){
      float* tVetP2= alocarVetorVet(n,m);
      for(int i=0;i<m;i++)
         for(int j=0; j<n;j++)
            tVetP2[j*m+i]= vet[i*n+j]; //*(vet+i*n+j)
return tVetP2;
}
float **transpostaMatizM(int m,int n,float **mat){
      float**tMatP2 = alocarMatrizM(n,m);// passa a coluna primeiro depois as linhas para alocação
       for(int i=0;i<m;i++)
         for(int j=0; j<n;j++)
            tMatP2[j][i]= mat[i][j];
return tMatP2;
