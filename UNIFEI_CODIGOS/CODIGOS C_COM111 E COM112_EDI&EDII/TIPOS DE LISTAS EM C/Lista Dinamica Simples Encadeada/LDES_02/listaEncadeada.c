...
int verifica_ordenaçao_simples(Lista *li){
    ...
    While(no!=NULL){
        if(no->aluno->matriculo < no2->aluno->matricula){
            ...
            no=NULL;//caso algum valor seja maior já saira do laço
            des=0;// indica que não esta ordenada  na onde crescente
        }
    }
    if(des==0){// se não esta ordenado na ordem crescente pode estar na decrescente
        no=(*li),no2=(*li)->prox;
        ...
        while(no!=NULL){
            if(no->aluno->matricula > no2->aluno->matricula){
                ...
                no=NULL;//se existir um valor que seja menor ja sai do laço
                des=0; // indica que não esta ordenado na ordem decrescente
            }
        }
    }
    return des; //1 esta crecente, 2 decrescente e 0 nenhuma ordenação
}
