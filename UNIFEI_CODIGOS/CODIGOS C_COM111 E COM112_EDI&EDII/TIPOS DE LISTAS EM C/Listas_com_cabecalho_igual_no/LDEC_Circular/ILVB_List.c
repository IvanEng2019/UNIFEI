...
int menu(){
    int op;
    printf("Operações com Lista circular:\n");
...
void iniciaNoCabecalho(celula *cabeca){
      cabeca->prox=cabeca; //1ª celula aponta para nulo e nao tem conteudo
}
...
int tamanhoLista(celula *list){
  int cont=0;
  if(listaVazia(list)){
    for(celula* noTemp=list->prox;noTemp!=list;noTemp=noTemp->prox)
        cont++;
  }
  return cont;
}
...
void insereInicioLista(celula *list){
	...
	celula* auxVelha=list->prox; //definido cabeçalho que list->prox=list
	list->prox=nova;
	nova->prox=auxVelha;
}
...
void insereFimLista(celula *list){
	...
	if(listaVazia(list)==0)
        ...
        celula* noTemp;
	    for(noTemp=list->prox;noTemp->prox!=list;noTemp=noTemp->prox);
        noTemp->prox=nova;
	}
}
...
void insereDadoOrdenado(celula *list){
    if(!listaVazia(list))
       insereInicioLista(list);
        ...
        for(auxTemp=list->prox; auxTemp->prox!=list && auxTemp->conteudo<x;auxTemp=auxTemp->prox)
            noAnt=auxTemp;
...
celula *buscaUltimaPosicaoConteudo(celula *list,int x){
   celula *noTemp;
   for (noTemp=list->prox;noTemp->prox!=list && noTemp->conteudo!=x;noTemp=noTemp->prox);
   return noTemp;
}
///busca a posição anterior de um elemento especifico
celula *buscaPenultimaPosicaoConteudo(celula *list,int x){
   celula *penultimo;
   for(celula *ultimo=list->prox;ultimo->prox!=list && ultimo->conteudo!=x;ultimo=ultimo->prox)
       penultimo=ultimo;
   return penultimo;
}
...
celula *buscaPenultimaPosicaoLista(celula *list){
    celula *penultimo;
    for(celula *ultimo=list->prox;ultimo->prox!=list;ultimo=ultimo->prox)
        penultimo=ultimo;
    return penultimo;
}
...
celula *buscaUltimaPosicaoLista(celula *list){
   celula *noTemp;
   for (noTemp=list->prox;noTemp!=list;noTemp=noTemp->prox);
   return noTemp;
}
...
void removeCelulaInicio(celula *list){
	if(!listaVazia(list))
		...
		celula *ultima=buscaUltimaPosicaoLista(list);
		ultima->prox=list->prox;//noDel->prox;
		printf("DELETADO: %3d\n\n", noDel->conteudo);
		free(noDel);
	}
}
